...
async function action(args, opts, command) {
  ...
  if (input.length !== 0) {
    ...
    if (input[0] === '-') {
      ...
      for (let i = 0; i < input.length; i++) {
        try {
          const result = await optimizeFile(config, input[i], output[i]);
          results.push(result);
        } catch (error) {
          errors.push({ file: input[i], error });
          console.error(colors.red(`Error processing file '${input[i]}': ${error.message}`));
        }
      }
...
async function processDirectory(config, dir, files, output) {
  ...
  if (svgFilesDescriptions.length === 0) {
    throw new Error(`No SVG files have been found in '${dir}' directory.`);
  }
  
  const errors = [];
  
  for (const fileDescription of svgFilesDescriptions) {
    try {
      await optimizeFile(
        config,
      ...
      errors.push({ file: fileDescription.inputPath, error });
      console.error(colors.red(`Error processing file '${fileDescription.inputPath}': ${error.message}`));
    }
  }
  
  if (errors.length > 0) {
    console.error(colors.red(`\n${errors.length} file(s) failed to process in directory '${dir}'.`));
  }
}
 ...
 * @return {Promise<any>}
 */
async function optimizeFile(config, file, output) {
  try {
    const data = await fs.promises.readFile(file, 'utf8');
    await processSVGData(config, { path: file }, data, output, file);
  } catch (error) {
    // 检查是否是目录错误
    if (error.code === 'EISDIR') {
      await optimizeFolder(config, file, output);
    } else if (error.code === 'ENOENT') {
      throw new Error(`Error: no such file or directory '${error.path || file}'.`);
    } else if (error instanceof SvgoParserError) {
      // 处理 SvgoParserError，保留详细信息并添加文件名
      const errorWithFile = new Error(`Error in file '${file}': ${error.toString()}`);
      errorWithFile.name = error.name;
      throw errorWithFile;
    } else {
      // 确保其他错误信息包含文件名
 ...
 * @return {Promise<any>}
 */
async function processSVGData(config, info, data, output, input) {
  ...
  try {
    await writeOutput(input, output, result.data);
    ...
    if (error.code === 'ENOTDIR') {
      throw new Error(`Error: output '${output}' is not a directory.`);
    }
    throw error;
  }
}
